...
// 要修改的值
function editallpage(val){

    chrome.tabs.query({}, function(tabs){ 

        for(var i =0;i<tabs.length;i++){
            chrome.tabs.sendMessage(tabs[i].id,{cmd:'editval',value:val},function(response) { });
        }
...
function delalls(){
    ...
    chrome.tabs.query({}, function(tabs){ 
        ...
        for(var i =0;i<tabs.length;i++){
            chrome.tabs.sendMessage(tabs[i].id,{cmd:'delall',value:"123"},function(response) { });
        }
