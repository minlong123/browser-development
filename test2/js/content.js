...
if(curhref.indexOf("jq22.com") != -1 && curhref.indexOf("code") != -1 ){
...
document.addEventListener('DOMContentLoaded', function()
{
	...
	// let biaoshi=curhref.split("/")[curhref.split("/").length-1];
	// return;
	if(curhref.indexOf("jq22.com") == -1){
		return;
	}

	// console.log(location.href+"页面第一次加载");
	// initTieba();
	if(curhref.indexOf('daima') != -1){
		...
		var arr=[];
		$('.cover-info>a').each(function(index,item){
		...
		setTimeout(function(){
			...
			let html='';
			html +='<!DOCTYPE html>\n<head>\n<meta charset='+"'utf-8'"+'>\n<title>纯净开发模板</title>\n';
			var jqbb = "https://www.sucait.cn/assets/libs/jquery/dist/jquery.min.js";
...
function sendMessageToBackground(type,message) {
	...
	// console.log(message);
	chrome.runtime.sendMessage({type:type,greeting:message}, function(response) {
		// tip('收到来自后台的回复：' + response);
		console.log("收到来自后台的回复");
		// console.log(response);
	});
