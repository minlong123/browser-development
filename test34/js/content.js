...
// let crrustatuss=chrome.storage.local.get("actionstatuss")

// console.log(curhref);
// console.log(curhref.split("/")[curhref.split("/").length-1]);
// let biaoshis=curhref.split("//")[1];
// let biaoshi=biaoshis.split('/').join("");
// biaoshi=biaoshi.split(".")[1];
// biaoshi=biaoshi.join("");
// console.log(biaoshi);
// 截图

sendMessageToBackground(7,"update storage")
...
document.addEventListener('DOMContentLoaded', function(){
	...
	// switch登录
	// https://www.twitch.tv/login?client_id=mlpjzuoqffzsj3obfbtwhlxeuepuvh&redirect_params=client_id%3Dmlpjzuoqffzsj3obfbtwhlxeuepuvh%26force_verify%3Dtrue%26redirect_uri%3Dhttps%253A%252F%252Faccounts.krafton.com%252Fauth%252Ftwitch%252Fcallback%26response_type%3Dcode%26scope%3Duser%253Aread%253Aemail%2Bviewing_activity_read%26state%3DKnHgda6CrmMFJ0xuk6qbvjaR
	if(curhref.indexOf("www.twitch.tv/login") != -1){
		...
		setTimeout(function(){
			...
			// page.fill("input[type=\"text\"]",steamuser)
			// page.fill("input[type=\"password\"]",steampass)
		
			// page.click("div[class=\"Layout-sc-nxg1ff-0 OZCSg\"]")
		},3000)

	}

// 登陆后绑定steam的脚本
// if(curhref.indexOf("https://accounts.krafton.com/") != -1 && curhref.indexOf("login") == -1 && curhref.indexOf("auth/steam") == -1 ){
	...
	// https://accounts.krafton.com/resend-activation
	if(curhref.indexOf("resend-activation") != -1){
...
function otherlocs(steam,switchs){
...
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse)
{
	...
	// steam初始化storage
	if(request.cmd == 'initstroages'){

		// console.log(initstroagess);
		otherlocs(request.datas,request.swtics);
